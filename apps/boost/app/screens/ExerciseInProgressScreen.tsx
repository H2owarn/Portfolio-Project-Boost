	...
	React.useEffect(() => {
		const prefetchImages = async () => {
			if (exercise.images && exercise.images.length > 0) {
				try {
					await Promise.all(exercise.images.map((imageUrl) => Image.prefetch(imageUrl)));
		...
		<>
			...
			<Screen scrollable={false} contentStyle={styles.container}>
				...
				<ScrollView style={styles.contentSection} contentContainerStyle={styles.contentPadding}>
					...
					<View style={styles.detailsCard}>
						...
						<View style={styles.rewardInfo}>
							<View style={styles.rewardItem}>
								...
								<Text style={[styles.rewardText, { color: palette.text }]}>+{exercise.xp_reward} XP</Text>
							</View>
							<View style={styles.rewardItem}>
								<MaterialIcons name="flash-on" size={20} color={palette.primary} />
								<Text style={[styles.rewardText, { color: palette.text }]}>-{exercise.stamina_cost} Stamina</Text>
							</View>
						</View>
					</View>
					...
					{/* Complete Button */}
					<Pressable
						style={[styles.completeButton, { backgroundColor: palette.primary }, completing && styles.buttonDisabled]}
						onPress={handleCompleteExercise}
						disabled={completing}
					>
						<MaterialIcons name="check-circle" size={24} color="#000" />
						<Text style={styles.completeButtonText}>{completing ? 'Saving...' : 'Complete Exercise'}</Text>
					</Pressable>
							...
							{exercise.instructions.map((instruction, index) => (
								<View key={`instruction-${index}`} style={styles.instructionItem}>
									<View style={[styles.stepNumber, { backgroundColor: palette.primary + '20' }]}>
										<Text style={[styles.stepNumberText, { color: palette.primary }]}>{index + 1}</Text>
									</View>
									<Text style={[styles.instructionFullText, { color: palette.mutedText }]}>{instruction}</Text>
								</View>
							))}
						</View>
					)}
